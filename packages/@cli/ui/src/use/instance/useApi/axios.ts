...
const decodeJwt = (jwt: string) => {
    try {
        ...
        const leftTime = data.exp - new Date().getTime() / 1000
        console.log('decodeJwt', leftTime)
        return {
            data: data.data,
            leftTime
        }
...
const saveCaptchaToken = (token: string) => {
    const data = decodeJwt(token)
    storage.save(`CaptchaToken-${data.data.type}`, token, data.leftTime)
}

const getCaptchaToken = (type: string) => {
    return storage.load(`CaptchaToken-${type}`)
}
...
const realRequest = async <T = any>(method: Method, url: string, opts?: UseApi.AxiosParams) => {
    ...
    if (options.captchaTokenType) {
        options.data = Object.assign({}, options.data, { hyCaptchaToken: getCaptchaToken(options.captchaTokenType) })
    }
...
const request = async <T = any>(method: Method, url: string, opts?: UseApi.AxiosParams): Promise<T> => {
